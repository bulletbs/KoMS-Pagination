			...
			OR isset($config['items_per_page']))
		{
			...
			else
			{
				...
				switch ($this->config['current_page']['source'])
				{
                    ...
                    case 'query_offset':
                        $this->current_page = 0;
                        $offset = $this->_request->query($this->config['current_page']['key']);
                        if($offset && $this->config['items_per_page'])
                            $this->current_page = (int) $offset / $this->config['items_per_page'];
                        break;
				}
			}
...
//        echo Debug::vars($this->_route_params);
		switch ($this->config['current_page']['source'])
		{
			case 'query_string':
